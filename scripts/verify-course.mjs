...
const regulations=JSON.parse(fs.readFileSync(new URL('../lib/regulations.json',import.meta.url),'utf8'));
...
assert.equal(regulations.length,86);
assert.deepEqual(regulations.reduce((all,row)=>{all[row.category]=(all[row.category]||0)+1;return all;},{}),{anhui:13,national:40,suzhou:33});
assert.equal(regulations.filter(row=>row.localTextUrl).length,85);
for(const row of regulations.filter(row=>row.localTextUrl))assert.ok(fs.existsSync(new URL('../public'+row.localTextUrl,import.meta.url)));
console.log('PASS: 18 teaching units plus 86 national, Anhui, and Suzhou policy records with collected source files.');
